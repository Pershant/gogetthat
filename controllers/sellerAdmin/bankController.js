...
const model = 'bank';
const modelBaseName = 'bank';
const modelTitle = 'Bank';
const modelImageFolder = 'bank';
...
module.exports = {
    listing: async (req, res) => {
        try {
            ...
            const data = await Promise.all(listing.map(async (row, index) => {
                ...
                const deleteButton = `<button model_id="${row.id}" model="${model}" model_title="${modelTitle}" datatable="${modelDataTable}" class="btn btn-outline-danger disable_btn" >Delete</button>`;
    ...
    add: async (req, res) => {
        try {
            ...
            return res.render(`${prefixBaseName}/${modelBaseName}/addEdit`, { row: undefined, prefixBaseName, modelBaseName, shopCategories:shopCategories });
        } catch (err) {
            return helper.error(res, err);
        }
    },
    ...
    delete: async(id)=>{
        return await model[model].destroy({
            where:{id},
