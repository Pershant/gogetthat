...
// const { firstDefined } = require('fullcalendar/src/util');
...
module.exports = {
  vaildObject: async function (required, nonRequired) {
    ...
    // Checking roles who can access this api, role column is mentioned in the user table. Just pass roles array like roles: [1,3] with req param inside required.
    if (required.hasOwnProperty('req') && required.hasOwnProperty('roles') && Array.isArray(required.roles) && !required.roles.includes(required.req.user.role)) throw "Invalid role to access this API.";
  ...
  save: async (model, data, returnData = false, req = {}) => {
    try {
      ...
      console.log(model, '===================>saving model');
      ...
      let id;
      // console.log(data, '===========================>data for updating in helper save method');
      // return;

      if (data.hasOwnProperty('id')) {
        console.log(data, '===========>data here inside id');
      ...
      if (returnData) {
        console.log('==>here inside return data');
  ...
  success: function (res, message = 'Update Successfully', body = {}) {
  ...
  imageUploadMultiple: (files, folder = 'users') => {
  ...
  sendOrderPushNotification: async function (model, type, order_id) {
    const findorder = await model.findOne({
      attributes: ['id', 'orderNo', 'customerId', 'vendorId',
        [sequelize.literal('(SELECT deviceToken FROM userToken WHERE userId = order.customerId order by id desc limit 1)'), 'device_token'],
        [sequelize.literal('(SELECT deviceToken FROM userToken WHERE userId = order.customerId order by id desc limit 1)'), 'deviceToken']
      ],
      where: {
        id: order_id
      },
    ...
    var pushData = {
      "device_token": findorder.deviceToken,
      "msg": "Order number " + findorder.orderNo + " " + type + " by vendor",
      "body": {
        "order_id": order_id,
    ...
    this.sendPushNotificationTifiFunction(pushData)
    var dataToSave = {
      userId: findorder.vendorId,
      user2Id: findorder.customerId,
      orderId: order_id,
      // type:,
      message: "Order number " + findorder.orderNo + " " + type + " by vendor"
    }
    this.save(models['notification'], dataToSave);
    return true;
  },

  sendAddOrderPushNotification: async function (model, vendorModel, type, order_id) {
    const findorder = await model.findOne({
      attributes: ['id', 'orderNo', 'vendorId', 'customerId',
        [sequelize.literal('(SELECT deviceToken FROM userToken WHERE userId = order.vendorId order by id desc limit 1)'), 'deviceToken'],
      ],
      where: {
        id: order_id
      },
    ...
    var pushData = {
      "vendor_id": findorder.vendorId,
      "device_token": findorder.deviceToken,
      "msg": "New order request. Order number " + findorder.orderNo,
      "body": {
        "order_id": order_id,
        "notification_type": type,
        "msg": "New order request. Order number " + findorder.orderNo,
    ...
    console.log("vendor pushData --------------------- ", pushData)
    this.sendPushNotificationTifiFunction(pushData)
    var dataToSave = {
      userId: findorder.customerId,
      user2Id: findorder.vendorId,
      orderId: order_id,
      message: "New order request. Order number " + findorder.orderNo
    }
    this.save(models['notification'], dataToSave);


    let get_staff = await vendorModel.findAll({
      attributes: ['id', 'userId', 'vendorId', 'customerId',
        [sequelize.literal('(SELECT deviceToken FROM userToken WHERE userId = vendorStaffDetail.userId order by id desc limit 1)'), 'deviceToken'],
      ],
      where: {
    ...
    if (get_staff && get_staff.length > 0) {
      for (let i in get_staff) {
        var epushData = {};
        var epushData = {
          "staff_id": get_staff[i].userId,
          "device_token": get_staff[i].deviceToken,
          "msg": "New order request. Order number " + findorder.orderNo,
          "body": {
            "order_id": order_id,
            "notification_type": type,
        ...
        console.log("staff epushData @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ ", epushData)
        this.sendPushNotificationTifiFunction(epushData)
        var dataToSave = {
          userId: findorder.customerId,
          user2Id: get_staff[i].userId,
          orderId: order_id,
          message: "New order request. Order number " + findorder.orderNo
        }
        this.save(models['notification'], dataToSave);
  ...
  sendPushNotificationTifiFunction: function (notification_data) {
    try {
      console.log("notification_data################33 ", notification_data)
      var serverKey = 'AAAAFnJtC-I:APA91bHFPiNGzfX93MyVh9cxDHtL6k-YNFUPw6hVSGxmyQQmzG3pWvq3V97cU3b4C5ljMZDp3XT9e8r3VxP9ZDM3PhVzkSSwSqKBj5TuU2yaYySpmwnx8tdcBC1WScaoDS1wzp-EhpgH';//'AAAA87hG_mQ:APA91bFubhzMoDS434ncH2B0H4686QFnD4xRws_KQME9uy-JT5-aQf1UG7zu_Q3IUP70xXIFqqhhvpXSiTifyiYTYU8QFaFYNqP3btQbmHCPEO98URWhDxW72IsIidCZ8HnuFNxFvZQh';
  ...
  makeImageUrlSqlForSubQuery: (field, modelFolder, returnField = field) => (`(IF (\`${field}\`='', '', CONCAT('${baseUrl}/uploads/${modelFolder}/', \`${field}\`)) )`),
  ...
  checkId: async (model, where, parameterName, error = true) => {
    ...
    const data = await model.findOne({
      ...(
        typeof where == 'object' ? {
          where
        } : typeof where == 'string' && !isNaN(Number(where)) ? {
          where: {
            id: parseInt(where)
          }
        } : {}
  ...
  makeImageUrlSqlsocket: (socket, model, field, modelFolder = 'user', returnField = field) => ([
    sequelize.literal(`(IF (LOCATE('http', \`${model}\`.\`${field}\`) > 0, \`${model}\`.\`${field}\`, IF (\`${model}\`.\`${field}\`='', '', CONCAT('${socket.handshake.headers.host}/uploads/${modelFolder}/', \`${model}\`.\`${field}\`)) ))`),
    returnField
  ]),
  ...
  send_push_notification: function (message, device_token,device_type,  data_to_push, push_type, user_image) {
    ...
    if (device_token != '' && device_token != null) {
      ...
      var fcm = new FCM(serverKey);
      if (device_type == 1) {
        // console.log(data_to_push,user_image,'===================here')
        var msg = { //this may vary according to the message type (single recipient, multicast, topic, et cetera)
          to: device_token,
  ...
  pushNotificationIos: async function (dataForSend) {
    try {
      ...
      // const topics = {
      // 	user: 'com.debug.renourish',
      // 	driver: 'com.debug.renourish',
      // }

      // const type = topicTypes[parseInt(dataForSend.type)];
      // const topic = "com.debug.renourish";
      const topic = "com.turkeyontheloose.app";
  ...
  pushNotificationAndroid: async function (notificationData) {
    try {
      ...
      // return;
      var serverKey =
        "AAAAFnJtC-I:APA91bHFPiNGzfX93MyVh9cxDHtL6k-YNFUPw6hVSGxmyQQmzG3pWvq3V97cU3b4C5ljMZDp3XT9e8r3VxP9ZDM3PhVzkSSwSqKBj5TuU2yaYySpmwnx8tdcBC1WScaoDS1wzp-EhpgH";
